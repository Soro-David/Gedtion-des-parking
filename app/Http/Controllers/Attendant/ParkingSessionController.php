...
class ParkingSessionController extends Controller
{
    public function index()
    {
        $sessions = ParkingSession::with('parking', 'agent')
            ...
            // dd($sessions);
        return Inertia::render('Attendant/ParkingSessions/Index', [
            'sessions' => $sessions,
        ]);
    }
    ...
    public function create()
    {
        ...
        return Inertia::render('Attendant/ParkingSessions/Create', [
            'parkings' => $parkings,
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        // Vérifier que le parking appartient bien aux parkings affectés à l'agent
        $assignedIds = auth()->user()->parkings()->pluck('parkings.id')->toArray();
        if (! in_array((int) $request->parking_id, $assignedIds)) {
            Log::warning('Accès non autorisé au parking (agent)', [
        ...
        if ($parking->available_spots <= 0) {
            Log::warning('Tentative de stationnement sur parking complet (agent)', [
        ...
        Log::info('Session de stationnement enregistrée (agent)', [
            'parking_id'    => $parking->id,
            'license_plate' => strtoupper($request->license_plate),
            'agent_id'      => auth()->id(),
        ]);

        return redirect()->route('attendant.parking-sessions.ticket', $session->id);
    }
    ...
    public function checkout(Request $request)
    {
        ...
        return Inertia::render('Attendant/ParkingSessions/Checkout', [
            'plate'    => $plate,
            'sessions' => $sessions,
            'rates'    => $rates,
        ]);
    ...
    public function checkoutStore(Request $request)
    {
        ...
        Log::info('Sortie enregistrée (agent)', [
        ...
        return redirect()->route('attendant.parking-sessions.receipt', $session->id);
    }
    ...
    /**
     * Historique des sorties enregistrées par cet agent.
     */
    public function history()
    {
        ...
        return Inertia::render('Attendant/ParkingSessions/History', [
            'sessions' => $sessions,
        ]);
    }
}
