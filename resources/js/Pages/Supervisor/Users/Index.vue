...
const routeForRole = {
    attendant: {
        edit:   'supervisor.attendants.edit',
        destroy:'supervisor.attendants.destroy',
        resend: null,
    },
    caissier: {
        edit:   'supervisor.caissiers.edit',
        show:   'supervisor.caissiers.show',
        destroy:'supervisor.caissiers.destroy',
        resend: 'supervisor.caissiers.resend-invitation',
    },
};

const confirmDelete = (user) => {
    if (confirm(`Voulez-vous vraiment supprimer ${user.name} ${user.first_name} ?`)) {
        router.delete(route('supervisor.users.destroy', user.id));
    }
};

const resendInvitation = (user) => {
    const r = routeForRole[user.role]?.resend;
    if (!r) return;
    if (confirm(`Renvoyer l'email d'invitation à ${user.name} ?`)) {
        router.post(route(r, user.id));
    }
};
...
const roleOptions = [
    ...
    { value: 'caissier',  label: 'Caissier(ère)',    description: 'Gère les paiements et la caisse du parking.' },
];
...
const submit = () => {
    form.post(route('supervisor.users.store'), {
        preserveState: true,
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">
                    {{ users.length }} utilisateur{{ users.length !== 1 ? 's' : '' }} sous votre supervision.
                </p>
            </div>
                        ...
                        <tr v-for="user in paged" :key="user.id" class="hover:bg-gray-50/50 transition-colors group">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    <!-- Voir (caissier uniquement) -->
                                    <Link
                                        v-if="user.role === 'caissier'"
                                        :href="route('supervisor.caissiers.show', user.id)"
                                    ...
                                    <!-- Renvoyer invitation -->
                                    <button
                                        v-if="!user.has_password && routeForRole[user.role]?.resend"
    ...
    <Modal :show="showModal" max-width="2xl" @close="closeModal">
        <div class="bg-white rounded-xl overflow-hidden">
            ...
            <form @submit.prevent="submit" class="px-6 py-5 space-y-5 max-h-[75vh] overflow-y-auto">
                ...
                <!-- ── Rôle ─────────────────────────────────────────── -->
                <div>
                    <InputLabel for="sv_role" value="Rôle de l'utilisateur" class="mb-1.5" />
                    <div class="relative">
                        <select
                            id="sv_role"
                            v-model="form.role"
                ...
                >
                    <div v-if="needsParking">
                        ...
                        <p class="mt-1.5 text-xs text-gray-400 flex items-center gap-1">
                            ...
                            Seuls les parkings qui vous sont affectés sont affichés.
                        </p>
                    </div>
                </Transition>
