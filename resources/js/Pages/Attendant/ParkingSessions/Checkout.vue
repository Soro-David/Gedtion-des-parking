...
const doSearch = () => {
    router.get(route('attendant.checkout'), { plate: search.value }, { preserveState: true, replace: true });
};
...
const confirmCheckout = () => {
    checkoutForm.post(route('attendant.checkout.store'), {
            ...
            <div v-if="selected" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" @click.self="closeModal">
                <div class="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-md mx-4">
                    ...
                    <div class="flex gap-3">
                        <button @click="closeModal" class="flex-1 px-4 py-2.5 border border-gray-200 rounded-xl text-sm font-semibold text-gray-700 hover:bg-gray-50 transition-colors">
                            Annuler
                        </button>
                        <button
