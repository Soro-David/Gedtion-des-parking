...
const allTotal = computed(() =>
    props.sessions.reduce((sum, s) => sum + (s.amount ?? 0), 0)
);
...
<template>
    <Head title="Historique caisse" />

    <AuthenticatedLayout>
        <template #header>Historique caisse</template>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <div>
                <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Historique caisse</h2>
                <p class="text-sm text-gray-500 mt-1">Toutes les sorties que vous avez encaissées.</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-center gap-4">
                ...
                <div>
                    <p class="text-xs text-gray-500 font-medium">Encaissements aujourd'hui</p>
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-center gap-4">
                ...
                <div>
                    <p class="text-xs text-gray-500 font-medium">Total encaissé (tout)</p>
                    <p class="text-2xl font-black text-purple-700">{{ allTotal.toLocaleString('fr-FR') }} <span class="text-sm font-semibold">FCFA</span></p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100">
                <span class="text-sm font-semibold text-gray-700">
                    <span class="text-gray-900 font-bold">{{ filtered.length }}</span> encaissement(s)
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50/50">
                            ...
                            <th class="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider text-right">Montant encaissé</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr v-if="filtered.length === 0 && sessions.length > 0">
                            <td colspan="8" class="px-6 py-16 text-center text-gray-400 italic">
                                Aucun encaissement trouvé pour « {{ search }} ».
                            </td>
                        </tr>
                        <tr v-if="sessions.length === 0">
                            <td colspan="8" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center gap-3 text-gray-400">
                                    ...
                                    <span class="font-medium">Aucun encaissement enregistré pour le moment.</span>
                                </div>
                            </td>
                        </tr>
                    </tbody>
