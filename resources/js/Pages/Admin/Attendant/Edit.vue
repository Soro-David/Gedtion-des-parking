...
const form = useForm({
    ...
    emergency_contact: props.agent.emergency_contact ?? '',
    emergency_relationship: props.agent.emergency_relationship ?? '',
    emergency_name: props.agent.emergency_name ?? '',
    parking_ids: (props.agent.parkings ?? []).map((p) => p.id),
});

const submit = () => {
    form.put(route('admin.agent.update', props.agent.id));
};
</script>
        ...
        <div class="max-w-4xl mx-auto pb-20">
            <div class="flex items-center gap-3 mb-8">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-0.5">Mettez à jour les informations de l'agent.</p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
                <form @submit.prevent="submit" class="p-8 space-y-8">
                    ...
                    <div class="pt-8 border-t border-gray-100 flex items-center justify-between gap-4">
                        <CancelButton :href="route('admin.agent.index')" />
