...
const confirmDelete = (id) => {
    if (confirm('Voulez-vous vraiment supprimer ce caissier ?')) {
        router.delete(route('admin.caissier.destroy', id));
    }
};

const resendInvitation = (id) => {
    if (confirm('Renvoyer l\'email d\'invitation à ce caissier ?')) {
        router.post(route('admin.caissier.resend-invitation', id));
    }
};
</script>

<template>
    <Head title="Gestion des Caissiers" />
    ...
    <AuthenticatedLayout>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Gérez les caissiers et leurs informations.</p>
            </div>
            
            <PrimaryLinkButton
                :href="route('admin.caissier.create')"
                label="Ajouter un Caissier(ère)"
                        ...
                        <tr v-for="caissier in pagedCaissiers" :key="caissier.id" class="hover:bg-gray-50/50 transition-colors group">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center justify-center gap-1">
                                    <!-- Show -->
                                    <Link
                                        :href="route('admin.caissier.show', caissier.id)"
                                    ...
                                    <!-- Edit -->
                                    <Link
                                        :href="route('admin.caissier.edit', caissier.id)"
